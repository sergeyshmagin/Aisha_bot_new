...
class ProgressHandler:
    ...
    async def update_gallery_if_open(self, user_id: int, avatar_id: UUID, photos: List[AvatarPhoto]):
        ...
        try:
            ...
            gallery_handler = PhotoUploadGalleryHandler()
            cache_data = await gallery_handler._get_gallery_cache(user_id)
            
            if cache_data and str(cache_data.get("avatar_id")) == str(avatar_id):
                # Обновляем кэш с новыми фотографиями
                updated_photos = [
                    {"id": str(p.id), "minio_key": p.minio_key, "width": p.width, "height": p.height, 
                     "created_at": p.created_at.isoformat()} for p in photos
                ]
                cache_data["photos"] = updated_photos
                await gallery_handler._set_gallery_cache(user_id, cache_data)
