...
from frontend_bot.services.minio_client import upload_file, download_file, ensure_bucket_exists, delete_file
import warnings
...
async def remove_photo_from_avatar(db: AsyncSession, user_id: int, avatar_id: str, photo_id: str) -> bool:
    ...
    try:
        ...
        if not avatar:
            logger.error(f"Аватар не найден: user_id={user_id}, avatar_id={avatar_id}")
            return False
        
        # Получаем фото
        photo_query = select(UserAvatarPhoto).where(
        ...
        if not photo:
            logger.error(f"Фото не найдено: photo_id={photo_id}")
            return False
            
        # Удаляем из MinIO
        try:
            await delete_file("avatars", photo.photo_key)
        except S3Error as e:
            logger.error(f"Ошибка при удалении из MinIO: {e}")
            # Продолжаем удаление из БД даже при ошибке MinIO
            
        # Удаляем из БД
        await db.delete(photo)
        await db.commit()
        
        logger.info(f"Успешно удалено фото: photo_id={photo_id}, avatar_id={avatar_id}")
        return True
...
async def cleanup_old_drafts(user_id: int, max_age_hours: int = 24) -> None:
    """
    DEPRECATED: Используйте cleanup_old_drafts из avatar_workflow.py
    Очищает старые черновики аватаров.
    Оставлено только для обратной совместимости. Будет удалено после миграции.
    """
    warnings.warn(
        "[DEPRECATED] Используйте cleanup_old_drafts из avatar_workflow.py. Эта функция будет удалена.",
        DeprecationWarning
    )
    try:
