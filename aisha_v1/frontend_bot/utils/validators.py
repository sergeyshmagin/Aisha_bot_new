...
def validate_user_avatar(handler):
    ...
    async def wrapper(call_or_message, *args, **kwargs):
        ...
        # Получаем avatar_id из callback_data
        avatar_id = None
        if hasattr(call_or_message, "data"):
            # Получаем avatar_id из state для удаления фото
            if call_or_message.data.startswith("avatar_gallery_delete"):
                from frontend_bot.services.avatar_manager import get_current_avatar_id
                avatar_id = get_current_avatar_id(user_id)
            else:
                # Для остальных операций берем из callback_data
                parts = call_or_message.data.split(":")
                avatar_id = parts[1] if len(parts) > 1 and parts[1] != "None" else None
        ...
        async with AsyncSessionLocal() as session:
            ...
            # Если avatar_id не получен из callback_data, пробуем взять текущий
            if not avatar_id:
                from frontend_bot.services.avatar_manager import get_current_avatar_id
