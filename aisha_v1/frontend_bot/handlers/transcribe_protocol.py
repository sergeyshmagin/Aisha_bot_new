...
async def send_meeting_protocol(message: Message):
    ...
    async with AsyncSessionLocal() as session:
        ...
        try:
            ...
            filename = (
                f"protocol_{uuid_user_id}_{datetime.now().strftime('%Y-%m-%d_%H-%M')}.docx"
            )
            ...
            # Добавляем запись в историю через HistoryService
    ...
    async with AsyncSessionLocal() as session:
        ...
        try:
            ...
            if not summary.strip():
                ...
                return
            filename = f"summary_{uuid_user_id}_{datetime.now().strftime('%Y-%m-%d_%H-%M')}.txt"
...
async def send_mom(message: Message) -> None:
    ...
    try:
        ...
        async with AsyncSessionLocal() as session:
            ...
            temp_file = f"mom_{uuid_user_id}_{datetime.now().strftime('%Y-%m-%d_%H-%M')}.txt"
...
async def send_todo_checklist(message: Message):
    ...
    async with AsyncSessionLocal() as session:
        ...
        try:
            ...
            filename = f"todo_{uuid_user_id}_{datetime.now().strftime('%Y-%m-%d_%H-%M')}.txt"
...
async def send_full_official_transcript(message: Message):
    ...
    async with AsyncSessionLocal() as session:
        ...
        try:
            ...
            filename = (
                f"full_transcript_{uuid_user_id}_{datetime.now().strftime('%Y-%m-%d_%H-%M')}.txt"
            )
...
async def handle_transcribe_file(message: Message, file_data: bytes, file_name: str) -> None:
    ...
    async with AsyncSessionLocal() as session:
        ...
        if not user:
            return
        uuid_user_id = user.id
        file_path = await save_transcribe_file(uuid_user_id, file_data, file_name, STORAGE_DIR)
...
async def protocol_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    Генерирует протокол из последней транскрипции пользователя.
    """
    try:
        ...
        async with AsyncSessionLocal() as session:
            ...
            content = transcript.content.decode("utf-8")
            
            # Генерируем протокол
            protocol = await generate_protocol(content)
            
            # Создаем клавиатуру с кнопками
            keyboard = [
                [
                    InlineKeyboardButton("📝 Редактировать", callback_data="edit_protocol"),
                    InlineKeyboardButton("📥 Сохранить", callback_data="save_protocol")
                ]
            ]
        ...
        logger.exception(f"[protocol_handler] Ошибка: {e}")
        await update.message.reply_text("❌ Произошла ошибка при генерации протокола")

async def generate_protocol(content: str) -> str:
    """
    Генерирует протокол из текста транскрипции.
    """
    # TODO: Реализовать генерацию протокола с помощью GPT
    return f"Протокол по транскрипции:\n\n{content}"
