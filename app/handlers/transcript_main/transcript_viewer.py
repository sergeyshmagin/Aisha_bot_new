...
class TranscriptViewer:
    ...
    async def get_transcript_with_content(self, user_id: str, transcript_id: UUID) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            async with self.get_session() as session:
                ...
                if not transcript:
                    logger.warning(f"Транскрипт {transcript_id} не найден для пользователя {user_id}")
                    return None
                ...
                if content:
                    try:
                        ...
                        logger.warning(f"Не удалось декодировать содержимое транскрипта {transcript_id}: {decode_error}")
                
                return transcript
                
        except Exception as e:
            logger.error(f"Ошибка при получении транскрипта {transcript_id} для пользователя {user_id}: {e}")
            return None
    
    def render_transcript_card(self, transcript: Dict[str, Any]) -> str:
        """
        Рендерит карточку транскрипта для отображения
        
        Args:
            transcript: Данные транскрипта
            
        Returns:
            HTML-текст карточки транскрипта
        """
        try:
            ...
            return TranscriptProcessingHandler().render_transcript_card(transcript)
        except Exception as e:
    ...
    async def open_transcript_by_callback(
        self, 
        call: CallbackQuery, 
        user_id: str
    ) -> bool:
        ...
        try:
            ...
            logger.error(f"Ошибка при открытии транскрипта (callback) для пользователя {user_id}: {e}")
            await call.answer("Ошибка при открытии транскрипта", show_alert=True)
            return False
    
    async def open_transcript_by_command(
        self, 
        message: Message, 
        user_id: str
    ) -> bool:
        ...
        try:
            ...
            logger.info(f"Транскрипт {transcript_id} успешно открыт по команде для пользователя {user_id}")
            return True
            
        except Exception as e:
            logger.error(f"Ошибка при открытии транскрипта (команда) для пользователя {user_id}: {e}")
