    ...
    async def start_generation(
        ...
        try:
            ...
            avatar = await self.get_avatar_by_id(
                avatar_id, user_id=user.id, message=message, show_error=False
            )
            if not avatar:
                await self.safe_clear_state(state)
                return
            ...
            # Проверяем баланс
            if not await self.check_user_balance(user, GENERATION_COST, message=message):
                await self.safe_clear_state(state)
                return
...
💡 Обычно занимает 30-60 секунд""",
            ...
            if not generation:
                await generation_message.edit_text(
                    "❌ Не удалось запустить генерацию. Попробуйте еще раз.", parse_mode="HTML"
                )
                await self.safe_clear_state(state)
                return
...
💡 Результат появится здесь автоматически""",
            ...
            await self.monitor_generation_status(
                generation_message, generation, custom_prompt, avatar_name
            )
    ...
    async def start_generation_from_callback(
        self, callback: CallbackQuery, state: FSMContext, aspect_ratio: str
    ):
        ...
        try:
            ...
            # Получаем аватар
            avatar = await self.get_avatar_by_id(
                avatar_id, user_id=user.id, callback=callback, show_error=False
            )
            if not avatar:
                await self.safe_clear_state(state)
                return
            ...
            # Проверяем баланс
            if not await self.check_user_balance(user, GENERATION_COST, callback=callback):
                await self.safe_clear_state(state)
                return
...
💡 Результат появится здесь автоматически""",
            ...
            # Запускаем мониторинг
            await self.monitor_generation_status(
                callback.message, generation, custom_prompt, avatar_name
            )
    ...
    async def monitor_generation_status(
        self, message, generation, original_prompt: str, avatar_name: str
    ):
        ...
        while attempt < max_attempts:
            try:
                ...
                current_generation = await self.generation_service.get_generation_by_id(
                    generation.id
                )

                if not current_generation:
                    await message.edit_text("❌ Ошибка: генерация не найдена", parse_mode="HTML")
                    return

                if current_generation.status == GenerationStatus.COMPLETED:
                    # Генерация завершена - показываем результат
                    await self.show_final_result(
                        message, current_generation, original_prompt, avatar_name
                    )
                    return
    ...
    async def show_final_result(self, message, generation, original_prompt: str, avatar_name: str):
        ...
        try:
            if not generation.result_urls or len(generation.result_urls) == 0:
                await message.edit_text("❌ Результат генерации недоступен", parse_mode="HTML")
                return

            duration = (
                (generation.completed_at - generation.created_at).total_seconds()
...
🎉 Ваше изображение создано!"""
            ...
            try:
                ...
                await message.reply_photo(
                    photo=result_url, caption=text, reply_markup=keyboard, parse_mode="HTML"
                )
                    ...
                    async with aiohttp.ClientSession() as session:
                        async with session.get(result_url) as response:
                            if response.status == 200:
                                ...
                                logger.info(
                                    f"Изображение успешно отправлено как файл для генерации {generation.id}"
                                )

                            else:
                                raise Exception(
                                    f"HTTP {response.status} при скачивании изображения"
                                )
...
• Обратитесь в поддержку"""
        ...
        keyboard = InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="🔄 Попробовать снова", callback_data="generation_menu"
                    )
                ],
