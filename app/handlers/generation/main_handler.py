...
Детальный промпт дает AI точные инструкции о композиции, освещении, технических параметрах и качестве, что значительно улучшает результат генерации."""
            ...
            # Проверяем, можно ли редактировать сообщение (текстовое ли оно)
            try:
                ...
                if "there is no text in the message to edit" in str(e):
                    # Если нельзя редактировать (это сообщение с изображением), отправляем новое
...
🚫 <b>Причина:</b> {analysis_result['error']}
...
✍️ <b>Созданный промпт:</b>
{analysis_result['prompt'][:150]}{'...' if len(analysis_result['prompt']) > 150 else ''}
...
📐 <b>Выберите размер изображения...</b>""",
    ...
    async def _show_photo_aspect_ratio_selection(self, message: Message, state: FSMContext, analysis_result: dict):
        ...
        try:
            ...
            prompt_text = str(custom_prompt) if custom_prompt else "Промпт не создан"
...
🎯 <b>Доступные размеры:</b>"""
    ...
    async def _start_photo_generation(self, message: Message, state: FSMContext, aspect_ratio: str, analysis_result: dict):
        """Запускает генерацию для фото-анализа с революционными улучшениями"""
