...
class BalanceService(BaseService):
    ...
    def __init__(self, session):
        ...
        # Создаем UserService с переданной сессией
        self.user_service = get_user_service_with_session(session)
    ...
    async def get_balance(self, user_id: UUID) -> float:
        ...
        """
        try:
            # Используем существующую сессию вместо создания новой
    ...
    async def charge_balance(
        ...
        try:
            ...
            current_balance = await self.user_service.get_user_balance(user_id)
            ...
            # Списываем средства
            new_balance = await self.user_service.remove_coins(user_id, amount)
    ...
    async def add_balance(
        ...
        try:
            ...
            # Добавляем средства
            new_balance = await self.user_service.add_coins(user_id, amount)
