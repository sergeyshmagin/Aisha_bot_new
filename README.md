...
- 🤖 **AI форматирование** - создание кратких содержаний, списков задач и протоколов через OpenAI
...
Меню транскрибации
    ...
    │   ├── < 1 мин → Бесплатная транскрипция
    │   └── > 1 мин → Платная транскрипция
    │       ├── 💰 Оплатить (при достаточном балансе)
    │       ├── 💳 Пополнить баланс  
    │       └── 🎁 Ввести промокод
    ├── 📝 Текст          # Обработка .txt файлов
    └── 📜 История        # Просмотр сохраненных транскриптов
        ↓ Выбор транскрипта
    Результат транскрипции + inline меню:
        ├── 📝 Краткое содержание    # AI через OpenAI
        ├── ✅ Список задач          # AI через OpenAI
        ├── 📊 Протокол             # AI через OpenAI  
        ├── ⬅️ Назад в историю
        └── 🏠 В главное меню
```
