# Тексты для транскрибации, аудио и связанных сценариев

AUDIO_REPEAT_TEXT = (
    "Пожалуйста, отправьте аудиофайл или голосовое сообщение " "в этот чат ещё раз."
)

MAIN_MENU_TEXT = "Главное меню:"

AUDIO_PROCESSING_TEXT = "⏳ Обработка куска {current}/{total}..."

AUDIO_ERROR_TEXT = (
    "Что-то пошло не так. Попробуйте ещё раз или " "обратитесь в поддержку."
)

AUDIO_DONE_TEXT = "705 Расшифровка завершена!\n\n{snippet}...\n(текст обрезан)"

AUDIO_CHOOSE_ACTION_TEXT = "Выберите дальнейшее действие:"

NO_TRANSCRIPT_TEXT = (
    "Нет сохранённого транскрипта. Пожалуйста, отправьте аудиофайл " "ещё раз."
)

FULL_TRANSCRIPT_TEXT = "🤖 Формирую полный официальный транскрипт с помощью GPT..."

SHORT_SUMMARY_CAPTION = "📝 Сводка на 1 страницу"

MOM_CAPTION = "📝 MoM (Minutes of Meeting)"

TODO_CAPTION = "📝 ToDo-план с чеклистами"

FORMATS_INFO_TEXT = (
    "📚 Описание форматов:\n\n"
    "📝 Полный официальный транскрипт — структурированный текст встречи с "
    "выделением участников, тем и итогов.\n\n"
    "📄 Сводка на 1 страницу — краткое резюме для руководства.\n\n"
    "📋 MoM — протокол встречи с решениями и задачами.\n\n"
    "✅ ToDo-план — чеклист задач по итогам встречи.\n\n"
    "Выберите нужный формат ниже!"
)

PROTOCOL_CAPTION = "📄 Протокол заседания (Word)"

PROTOCOL_ERROR_TEXT = (
    "Что-то пошло не так при формировании протокола. "
    "Вы можете повторить попытку или выбрать другой формат."
)
