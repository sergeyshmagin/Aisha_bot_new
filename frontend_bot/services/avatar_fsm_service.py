...
from frontend_bot.services.state_utils import set_state, clear_state
...
async def set_avatar_type(user_id, avatar_id, gender, session):
    ...
    await set_state(user_id, {"state": "avatar_enter_name", "avatar_id": avatar_id}, session)

async def set_avatar_name(user_id, avatar_id, name, session):
    await update_draft_avatar(user_id, session, {"avatar_data": {"title": name}})
    await set_state(user_id, {"state": "avatar_confirm", "avatar_id": avatar_id}, session)
